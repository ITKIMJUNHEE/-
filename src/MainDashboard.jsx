...
import { Zap, AlertTriangle, Settings, BarChart3, ArrowRight, Sun, CloudRain, Snowflake, LogOut } from 'lucide-react';
    ...
    <div className="relative w-screen h-screen overflow-hidden font-sans text-slate-800 select-none bg-slate-50">
      ...
      <header className="absolute top-0 left-0 w-full p-6 z-50 pointer-events-none bg-gradient-to-b from-white/90 to-transparent flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex items-center gap-3">
          <div className="font-mono text-xs text-slate-400 bg-white/50 px-3 py-1 rounded-full hidden md:block"><p>DATA: 2025 LIVE</p></div>
          
          {/* ⭐ [NEW] 로그아웃 버튼 (로그인 화면으로 이동) */}
          <button 
            onClick={() => navigate('/')} 
            className="flex items-center gap-2 bg-white/80 border border-slate-200 text-slate-600 px-4 py-2.5 rounded-full font-bold shadow-sm hover:bg-slate-100 transition-all"
          >
            <LogOut size={16} /> 로그아웃
          </button>
      ...
      <aside className="absolute bottom-10 left-10 w-96 z-50 flex flex-col gap-4 hidden md:flex">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/50">
           <div className="flex items-center gap-2 mb-4 text-blue-700">
             ...
             <h2 className="font-bold text-lg tracking-wide">기상 조건 설정</h2>
           </div>
           ...
           {weather.type !== 'sunny' && (
             <div className="animate-fade-in">
               <div className="flex justify-between text-xs font-bold text-slate-600 mb-2">
                 <span>강도 ({weather.type === 'rain' ? '강수량' : '적설량'})</span>
                 <span className="text-blue-600">{weather.intensity} {weather.type === 'rain' ? 'mm' : 'cm'}</span>
               </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/50">
          ...
          <div className="mb-8 group">
            <div className="flex justify-between mb-3 text-sm font-bold text-slate-700">
              <span>트램 배차 간격</span><span className="text-blue-600 font-mono text-lg">{params.tramInterval}분</span>
            </div>
            <input type="range" min="3" max="15" step="1" value={params.tramInterval} onChange={(e) => setParams({...params, tramInterval: Number(e.target.value)})} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
          </div>
          <div className="group">
            <div className="flex justify-between mb-3 text-sm font-bold text-slate-700">
              <span>버스 노선 감축률</span><span className="text-red-500 font-mono text-lg">{params.busReduction}%</span>
            </div>
            <input type="range" min="0" max="50" step="5" value={params.busReduction} onChange={(e) => setParams({...params, busReduction: Number(e.target.value)})} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-red-500" />
          </div>
        </div>
      </aside>
      ...
      <aside className="absolute top-28 right-10 w-[400px] space-y-4 z-50 hidden md:block">
        <div className="bg-white/90 backdrop-blur-xl rounded-2xl p-6 shadow-xl border-l-4 border-blue-600">
          <div className="flex justify-between items-center mb-2">
            <span className="text-slate-500 text-sm font-bold">연간 운영 예산</span><Zap className="text-blue-600 w-5 h-5" />
          </div>
        ...
        <div className={`bg-white/90 backdrop-blur-xl rounded-2xl p-6 shadow-xl border-l-4 transition-colors duration-300 ${results?.congestion > 100 ? 'border-red-500' : 'border-emerald-500'}`}>
          ...
          <div className="flex items-end gap-3">
             <p className={`text-4xl font-black ${results?.congestion > 100 ? 'text-red-600' : 'text-slate-800'}`}>{results?.congestion}%</p>
             <span className={`text-sm font-bold px-2 py-1 rounded mb-1 ${results?.complaintRisk === '심각' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>{results?.complaintRisk} 단계</span>
          </div>
           ...
           <div className="h-40 w-full">
             <ResponsiveContainer width="100%" height="100%">
               <AreaChart data={results?.chartData}>
                 <defs><linearGradient id="colorVal" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={0.8}/><stop offset="95%" stopColor="#2563eb" stopOpacity={0}/></linearGradient></defs>
